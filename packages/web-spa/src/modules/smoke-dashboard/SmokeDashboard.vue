...
import { ListChecks, RefreshCw, Server } from '@lucide/vue'

import SmokeCard from './SmokeCard.vue'
import type { ReviewSkill } from './types'
import { useSmokeChecks } from './useSmokeChecks'

const { health, skills, skillsCount, isRefreshing, refresh } = useSmokeChecks()
    ...
    <div class="mx-auto max-w-6xl space-y-6">
      <header class="border-4 border-black bg-white p-5 shadow-[8px_8px_0_#000]">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <p class="font-mono text-sm font-bold uppercase tracking-[0.24em]">Backend status</p>
      ...
      <section class="grid gap-6 md:grid-cols-2">
        <div class="border-4 border-black bg-white p-5">
          <h2 class="text-xl font-black uppercase">What this can test now</h2>
          <ul class="mt-3 list-square space-y-2 pl-5 font-bold">
            <li>Server liveness through the health endpoint.</li>
            <li>Embedded review skill metadata exposed by the backend.</li>
          </ul>
        </div>
        <div class="border-4 border-black bg-white p-5">
